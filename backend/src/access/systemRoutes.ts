...
export function createSystemRoutes(llm: LLMService): express.Router {
    ...
    try {
      ...
      try {
        ...
        const totalTick = cpus.reduce((sum: number, cpu: any) =>
          sum + (Object.values(cpu.times) as number[]).reduce((a: number, b: number) => a + b, 0), 0);
      ...
      } catch { diskUsage = { total: 0, used: 0, percentage: 0 }; }
      ...
      const tokenYear = parseInt(req.query.tokenYear as string) || new Date().getFullYear();
      const latencyYear = parseInt(req.query.latencyYear as string) || new Date().getFullYear();
      ...
      const generateCalendarMatrix = (year: number, isToken: boolean): Array<{ date: string; tokens: number; calls: number; avgLatency: number }> => {
        ...
        for (let month = 0; month < 12; month++) {
          ...
          for (let day = 1; day <= daysInMonth; day++) {
            ...
            matrix.push({ date: dateStr, tokens: isToken ? tokens : 0, calls: isToken ? calls : 0, avgLatency: isToken ? 0 : avgLatency });
          }
        }
        return matrix;
      };
      ...
      const storageStatus = {
        ...
        relationalDb: { type: 'SQLite', path: getConfig().dbPath, status: 'active' },
        vectorDb: { type: 'Local File System', path: getConfig().vectorDbPath, status: 'active' },
        graphDb: { type: graphDbType, path: getConfig().graphDbPath, status: 'active' },
      };
