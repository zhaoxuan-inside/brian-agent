...
function ensureConfig(): Record<string, unknown> {
  ...
  if (!row) {
    const id = generateId();
    const now = Date.now();
    DB.prepare(`INSERT INTO agent_library_config (id, created, updated, prompt_template_id, similarity_threshold, max_agent_count)
      VALUES (?, ?, ?, ?, ?, ?)`).run(id, now, now, '', 0.7, 100);
    return { id, created: now, updated: now, prompt_template_id: '', similarity_threshold: 0.7, max_agent_count: 100 };
  }
  return row;
}
