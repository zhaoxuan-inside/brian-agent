...
### [2026-09-12] 任务特质差异化 Soul 匹配与自动生成闭环

**变更原因**：
当现有 Soul 库中仅有特定类型（如仅有通用编码助手），而用户提问涉及数学公式推导、文字创作、数据分析等不同领域时，LLM 打分均未达采纳阈值。原实现错误回退到库内首个编码人设，未能根据任务领域生成最匹配的专属特质 Soul。

**修改的方法**：
- `SoulCoreService.matchSoul` — 当库内可用 Soul 排序打分均低于采纳阈值（无合适人设）或库为空时，自动触发 `generateAndAddSoul`，依据任务领域（如数学、文字、编码）生成契合该任务特质的专属角色设定，并赋予标准 UUID 存入 `soul` 表；
- 选定或新生成的 `soul_id` 返回上层 Agent 构建，持久化至 `agent` 与 `runtime_agent_def` 表，并在运行时将该 Soul 注入至主代理统一身份声明的 `{{soul}}` 人格段中。
