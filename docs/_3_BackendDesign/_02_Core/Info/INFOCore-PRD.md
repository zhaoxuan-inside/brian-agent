...
5. **数量与比例双控制（2026-09-15 口径）**：除钉住消息外，各弱相关维度（标签 / 向量相似 / 关键词 / 随机）限额 = **min（该维度基础数量， 「基础上下文消息数量」（钉住 + 复选/时间线）× 对应上限百分比）**；百分比基准为**基础上下文数量**（2026-09-15 前：占比基准为 total 且叠加 shrinkFactor 动态收缩）。random_max_percent 默认 5%（基础 100 条 → 随机最多 5 条）；标签 20% / 相似度 15% / 关键词 10%；
...
### [2026-09-15] RANDOM 名额核算顺序修正 + 遗留 priority_order 数据修复
