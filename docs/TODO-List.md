...
**配套遗留**：① ~~`evalWriterAgent` 前置段与 `resolveEvalContext` 逐字重复~~ **已解决（2026-09-22 批次1）**——`resolveEvalContext` 入参放宽为结构子集后 `evalWriterAgent` 复用（EvolutorAgent-PRD 同日更新）；② IntentAgent `info_content` 恒 undefined 拼入历史 prompt（any 治理发现，修复需上游结构对齐）；③ `writeLLMCoreQuotaConfig` 传参缺 `llm_provider_id` 运行时会抛 ValidationError（any 治理发现，decisions.md [2026-09-22e] 登记）；④ Base/Core 双路径并存——上层（AgentDefService/ChatService/ConfigService 等）同时持有 Base Access 具体类与 Core Access，违反外部资源接入唯一性的精神，收口需逐模块裁决 canonical 路径（单独排期）；⑤ 目录结构偏差 ~8 处（ChunkProvider/MQCoreProvider 等缺 infrastructure、ToolProvider 根目录散落 4 个工具文件），移动文件涉及跨模块 import 调整，批次 1 拆分时顺带处理。
...
| 批次 1 | >120 行余量 9 个 | ✅ 已完成（2026-09-22）：getCurrentValue(164)、login(157)、soTagGraph(156)、listLLM(152)、evalWriterAgent(152)、generateProfile(133)、soResource(127)、soConfigDetail(124)、deleteSession(122) 全部拆至 ≤40 行编排 + 子方法；`120+` 由 11 降至 2。**AopProxy.wrap(166)/get(148) 两个横切基建单独排期**（拆分需同步回归全部 46 个接入点） |
| 批次 2 | 80–120 行 ~37 个 | 按层分组拆分（当前 ≤120 区间余量 38 个，Top：genLLMAttr 118、openChatStreamV2 111、soUserProfile 111、pushStream 108、embedLLM 107） |
...
| 3 编排即工具 | update_plan + delegate + ask_user；steering/队列模式；Evolutor → curator | ✅ 已完成（2026-09-22：Runs 两段式 + Agents 确定性匹配 + Loop 真队列 + 编排三件套 update_plan/delegate/ask_user + curator background lane 调度） |
| 4 网关切换 | RunGateway 两段式接管 HTTP；前端 v2 协议切换 | ✅ 已完成（2026-09-22：后端 Chat v2 分流 + 前端 v2 原生协议消费核实完毕——过渡投影已删、分发表纯 v2；ask_user 提问卡补齐，`/api/chat/ask/answer` 答复恢复） |
| 5 退役 | Runtime-PRD §10 退役清单全部下线；可视化改为事件投影 | 进行中（2026-09-22：Orchestration 全模块/IntentAgent 暂停/Writer JSON blocks 已退役并清理遗留接线（workspace/依赖/配置种子）；剩余 AgentExecution + PlannerAgent 需先完成「Visualization 事件投影重建 → Evolutor trace 迁移 → ConfigService 收口」三步前置，见 Runtime-PRD §10 退役前置清单） |
