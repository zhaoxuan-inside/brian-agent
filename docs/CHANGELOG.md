...
**修改的内容**：
  - `HomeView.vue` 记忆地图：卡片重排为**两行三列对齐网格**（列 x=40/340/640 等宽 250，行 y=50/250 等高 140，与 Hero 同款布局语义）；连线全部沿网格正交方向（问答链 solid、引用/勾选依据 dashed），无斜线穿越；卡片信息补全（时间戳、标题、副标、引用/被引用/思考过程/评估结果胶囊、字数、Pin 圆点），胶囊布局复用新抽出的共享工具。
  - **图例从绝对定位浮层移至画布下方页脚条**（原浮层遮挡 n4/n6 卡片），画布内零遮挡；viewBox 900×540 → 900×450。
  - 新增 `src/utils/cardChipLayout.ts`（chipWidthOf / layoutChipsInCard：卡片内胶囊自动换行 + 行底对齐），从 HeroAppShot 上提为两处地图共用的纯函数（支柱：同逻辑 ≥2 模块必须上提）；HeroAppShot 改为调用共享工具。
  - 新增 `test/cardChipLayout.test.ts`（3 用例：宽度公式 / 换行 / 底对齐）。

**影响的端点**：仅前端首页 `/` 记忆地图区块渲染，无接口变更。

**可能存在的问题**：
  - 卡片时间/胶囊/字数为示意文案（原 SVG 只有标题+副标），如后续要接真实数据需调整 MapCard 结构。

**验证**：vue-tsc 0 错误；eslint 0 problems；`npm run build` 通过（HomeView 57.63 kB）；vitest 120/125（新增 3 用例全绿，5 个失败为既有基线）；headless Chrome 目检：网格对齐、正交连线、图例页脚条、零遮挡。
