...
| `pushText` | `sessionId: string, event: string, text: string, meta?: { run_id?: string; work_id?: str...` | `Promise<boolean>` | 推送打字机文本片段（自动 2-5 字符 chunk 切片） |
| `pushEvent` | `sessionId: string, event: string, msgType: SSEMessageType, data: T, meta?: { run_id?: s...` | `Promise<boolean>` | 推送结构化事件对象（DAG事件、上下文事件、Agent规格事件、控制事件等） |
