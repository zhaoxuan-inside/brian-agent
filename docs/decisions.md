...
## [2026-09-24] Skill 沉淀判定：行为信号替代 LLM 语义猜想

**决策**：Skill 绑定与 need 解耦（本地合格候选即绑定）；need 降级为扩容门禁（纯对话拦截）；负缓存引入"重复即沉淀"计数——同任务第二次出现强制走全链扩容（GitHub/自建），失败写回负缓存形成间歇重试。
**原因**：trace 3eea3bea 第三次实证——judge 类"元问题"判定（需不需要/值不值得）被候选语境/保守偏置带偏是 LLM 系统性弱点，模板措辞约束力有限；用户重复提问是沉淀价值最可靠的行为信号。
**备选**：继续打磨判定模板措辞（弃用——三次实证失败模式相同）；删除 need 门禁全量扩容（弃用——每句闲聊都触发 GitHub/自建，成本失控）。

## [2026-09-24] 彻底消除 Tool 概念，由 Skill 与 MCP 全面承接

**决策**：彻底废除独立的「Tool」领域概念，系统能力唯一归一为 **Skill**（系统内置技能 `skill_builtin-*` + 沉淀技能 `skill_<id>`）与 **MCP**（外部通道 `mcp_exec`）；原 `Runtime/Tools` 重构为 `Runtime/SkillRuntime`；`skill_exec` 间接 gate 删除；OpenAI wire `tools` / DB 列 `tool_id` / SSE 事件名仅作为底层存储/协议细节保留。
**原因**：用户裁决 —— Tool 与 Skill 定位重复，双平行管线导致持续的认知与实现漂移。
**备选**：维持 Tool 为底层原子原语、Skill 为上层知识包（弃用——用户明确要求项目中不出现 Tool 概念）。
