...
const SKILL_TEMPLATE = `<skill_selection_protocol version="sedimentation-value-contract">
  <identity>
    ...
    <purpose>从候选 Skill 中选出贴合者；无贴合者时判定该任务模式是否值得沉淀为新的可复用 Skill</purpose>
  </identity>
  ...
  <judgement_rules>
    <rule priority="1">背景：命令执行、浏览器等原子工具已内建可用，任何任务模型都能凭原语完成。Skill 的独特价值是「可复用的过程性知识包」——把某类任务的成熟方法论（步骤、命令组合、解读要点、输出规范）沉淀为文档与脚本，令同类任务下次更准更快。</rule>
    <rule priority="2">candidates 打分只看与任务的契合度（0-100，>=90 才算合格）；无关候选一律不给分。</rule>
    <rule priority="3">need 是「沉淀价值判定」（与候选库无关，禁止因库存无匹配而判 false）：纯闲聊/一次性问答/简单直答 -> need=false；系统分析（磁盘/CPU/内存/网络/进程）、数据处理、检索调研、内容生成、自动化操作等「会重复出现且步骤较多」的任务模式 -> need=true，并提炼 1~5 个英文检索关键词。</rule>
    <rule priority="4">库存全无合格候选且 need=true -> 系统将从外部检索或自动生成新 Skill（沉淀方法论），下次同类任务直接复用。</rule>
  </judgement_rules>

  <output_contract>
    <format>{"need": true, "keywords": ["memory","usage","analysis"], "candidates": [{"id": "<候选skill_id>", "score": 85}]}</format>
    <instruction>仅输出上述 JSON 对象，不要任何解释文本；candidates 按 score 降序（0-100 百分制）；无关候选不输出。</instruction>
  </output_contract>
...
const AGENT_TEMPLATE = `<agent_match_protocol version="capability-aware-contract">
  ...
  <judgement_rules>
    ...
    <rule priority="4">purpose 为"覆盖通用领域/日常问答/自检/状态报告/问候"类泛化 Agent，对数据获取与命令执行类任务一律视为能力不足，score 不�过 0.4；若不存在能力相容者，agent_id 返回空字符串，由系统构建新 Agent。</rule>
...
function apply(templateId, brief, content, marker = CONTRACT_MARKER) {
  ...
  if (String(row.prompt_template).includes(marker)) {
...
const r1 = apply('251fef0b-5082-4767-aaaa-f39c4d75d7f5', 'Skill 匹配模板', SKILL_TEMPLATE, 'sedimentation-value-contract');
