...
export const BUILTIN_PROMPTS: BuiltinPromptDef[] = [
  {
    ...
    template: [
      ...
      'Tools: {{tools_json}}',
      'Iteration: {{iteration}}',
      'Reason step by step. If external tools are needed, set next_action.tool_type to SKILL, MCP or CDT with tool_id and params. CDT 为内置浏览器能力，tool_id 取 browser.operations 中的 id。Context 含用户本机运行环境：工具参数与路径必须匹配该系统。用户要的是最终结果时，优先实际产出而非跨平台教程。Return JSON: {"reasoning":"...","next_action":{"tool_type":"NONE|SKILL|MCP|CDT","tool_id":"","params":{},"sub_steps":[]}}',
  ...
  },
  {
    ...
    template: [
      ...
      'Tools: {{tools_json}}',
      'History: {{history}}',
      'Generate the final answer for the runtime environment in Context (current OS / home / downloads dir). Deliver the concrete result the user asked for (paths, files, lists, data). Do NOT include other operating systems or dual-column POSIX/Windows tables unless the user explicitly asked for a cross-platform guide. Follow-ups like "我需要的是最终结果" / "直接处理" mean: output the artifact now from conversation context.',
      '只输出必要内容，保持准确、完整、简洁，不输出多余说明。',
    ].join('\n'),
  },
  {
    ...
    template: [
      ...
      'Results:',
      '{{agent_results}}',
      'The Context includes the user machine runtime. Write the final response for THAT OS only.',
      'If the user asked to 处理/给出/列出/下载 a concrete result (especially file paths), lead with the actual paths on this machine. Do not add Windows/Linux counterparts when the runtime is macOS, and vice versa.',
      'Follow-ups that say they want the 最终结果 / 直接处理 mean: stop writing plans or dual-OS specs; emit the deliverable.',
  ...
  },
  {
    ...
    template: [
      ...
      '14. Conversation continuity: if Context already establishes a topic, platform, product, or entity (e.g. earlier turns discussed YouTube), treat follow-up questions as continuing that topic. Do NOT ask the user to re-choose among alternatives already implied (YouTube vs Bilibili, etc.). Referential phrases like "前十浏览量" / "那些视频" / "下载到本地" inherit the established topic. Only emit clarifications when the missing parameter is truly absent from BOTH the current task AND the conversation context, AND choosing it would materially change execution. If the request is merely underspecified but the session topic is clear, proceed with that topic instead of asking a multiple-choice question.',
      '15. Runtime environment: Context includes the user\'s current machine (OS, home, downloads dir). All executable subtasks MUST target that environment only. Do not create parallel Windows/Linux/macOS variants or dual-OS path mapping tasks.',
      '16. Deliverable orientation: If the user asks to 处理/给出/列出/下载/保存 a concrete result, the DAG outcome is that artifact on this machine — not a tutorial, comparison table, or "how you could do it". Follow-ups like "我需要的是最终结果" / "直接处理top10的下载文件路径" mean: reuse conversation context and emit the concrete deliverable now.',
      '',
  ...
  },
  {
    ...
    template: [
      ...
      '2. 当用户输入极度模糊、存在多种互斥解读、且无上下文可推断时，match_score 应显著低于 50；',
      '3. 当用户输入虽简短但可合理扩写为同一主题的可执行任务时，match_score 应不低于 70，不要因为扩写了细节而压低分数；',
      '4. 仅当改写可能偏离用户原意（换了主题或目标）时才给低分。',
      '只输出必要内容，保持准确、完整、简洁，不输出多余说明。',
    ].join('\n'),
  },
  {
