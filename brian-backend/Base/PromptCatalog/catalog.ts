...
export const PROMPT_IDS = {
  ...
  mcpMarketMatch: 'builtin.mcp_market_match',
...
export const BUILTIN_PROMPTS: BuiltinPromptDef[] = [
  ...
  },
  {
    ...
    title: 'Skill 匹配排序',
    brief: 'SkillCore 需求判定与排序合并（need/keywords/candidates 契约；need=false 负缓存，need=true 无合格候选触发 GitHub 搜索与自建）',
    variables: ['agent_id', 'task_content', 'skills'],
    template: [
      '你是一个 Skill 匹配评估助手。请先判断当前任务是否需要借助 Skill（可复用的能力包）才能完成；',
      '若任务（如闲聊、常识问答）无需任何 Skill 即可由模型直接完成，need 设为 false；',
      '否则 need 设为 true，并为每一个候选 Skill 打分（0~100 整数，100 表示完美匹配）。',
      '',
      '当前任务:',
      '{{task_content}}',
      '',
      ...
      '若 need=true 但列表中没有能胜任的 Skill，candidates 输出空数组，并在 keywords 中给出',
      '2~5 个能用于外部检索（GitHub）的英文关键词（从任务中提炼，如 "weather forecast api"）。',
      '请严格仅输出如下格式的 JSON 对象，不输出任何解释说明：',
      '{"need": true, "keywords": ["weather"], "candidates": [{"id": "skill 的 id", "score": 78}]}',
      'need=false 时 keywords 与 candidates 均输出空数组。',
    ].join('\n'),
  },
  {
    id: PROMPT_IDS.mcpMatch,
    title: 'MCP 匹配推荐',
    brief: 'MCPCore 需求判定与排序合并（need/candidates 契约；need=false 负缓存，need=true 无合格候选触发提供商市场获取）',
    variables: ['agent_id', 'task_content', 'available_mcps'],
    template: [
      '你是一个 MCP 工具匹配评估助手。请先判断当前任务是否需要借助外部 MCP 工具（如联网检索、',
      '数据库、第三方 API）才能完成；若任务无需任何 MCP 即可由模型直接完成，need 设为 false；',
      '否则 need 设为 true，并为每一个候选 MCP 打分（0~100 整数，100 表示完美匹配）。',
      '',
      '当前任务:',
      '{{task_content}}',
      '',
      ...
      '若 need=true 但列表中没有能胜任的 MCP，candidates 输出空数组。',
      '请严格仅输出如下格式的 JSON 对象，不输出任何解释说明：',
      '{"need": true, "keywords": [], "candidates": [{"id": "mcp 的 id", "score": 78}]}',
      'need=false 时 candidates 输出空数组。',
    ].join('\n'),
  },
  {
    id: PROMPT_IDS.mcpMarketMatch,
    title: 'MCP 市场匹配',
    brief: 'MCPCore 在提供商市场候选中为当前任务选出最合适的 MCP（旧数组契约，百分制打分）',
    variables: ['task_content', 'available_mcps'],
    template: [
      '你是一个 MCP 工具选型评估助手。当前任务在本地已安装的 MCP 中没有能胜任的工具，',
      '请从提供商市场候选 MCP 中选出最能胜任当前任务的一个，并打分（0~100 整数）。',
      '',
      '当前任务:',
      '{{task_content}}',
      '',
      '市场候选 MCP 列表（JSON，含 id、title 与 brief）:',
      '{{available_mcps}}',
      '',
      '评分标准：score 直接代表"该工具能胜任当前任务的程度"，无关联即为低分。',
      '请严格仅输出如下格式的 JSON 数组（分数最高的一个即为选型结果），不输出任何解释说明：',
      '[{"id": "候选 mcp 的 id", "score": 78}, {"id": "另一个候选的 id", "score": 41}]',
      '若没有任何候选能胜任任务，输出空数组 []。',
    ].join('\n'),
  },
  {
