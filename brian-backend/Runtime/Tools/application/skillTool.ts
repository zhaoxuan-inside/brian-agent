 ...
 * - 运行注册（run 作用域）：ToolService.runTools 注册表随 Loop 结束清理 —— 全局 registry 不被会话性工具污染。
 * - MCP 通道（mcp_exec + component_scope.mcps）保持独立，不受本次合并影响。
 */

import { z } from 'zod';
import type { AnyToolDef } from '../domain/types';
...
/** skill 工具 id（数据处理） */
export function skillToolId(skillId: string): string {
  return `${SKILL_TOOL_PREFIX}${skillId}`;
}
...
/** Skill 一等工具定义构建（数据处理）：每 skill 一个 ToolDef，execute 复用 execSkill */
export function toSkillToolDef(skillId: string, skill: SkillRecordLike, skillAccess: SkillAccessLike): AnyToolDef {
  const name = String(skill?.name ?? '').trim() || skillId;
  const brief = String(skill?.skill_brief ?? '').trim();
  const mdHint = mdHintOf(skill?.skill_md);
  return {
    id: skillToolId(skillId),
    description: [
    ...
    // 返回 ToolResult 形状（与全部内置工具一致 —— executeToolSafely.truncateResult 直访 result.output）
...
export async function buildSkillToolDefs(
  skillIds: string[],
  ...
  for (const id of skillIds) {
    try {
      ...
      if (ok && out.skill?.enable) {
        defs.push(toSkillToolDef(id, out.skill, skillAccess));
      }
    } catch {
