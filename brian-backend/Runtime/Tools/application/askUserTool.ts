...
export function askUserTool(deps: AskUserDeps): ToolDef<{ question: string; kind?: string }> {
  return {
    id: 'ask_user',
    description:
...
async function executeAskUser(
...
): Promise<{ status: ToolResultStatus; output: string }> {
  ...
  if (!result.answered || !result.answer.trim()) {
    return { status: ToolResultStatus.Error, output: '用户未及时答复（等待超时）。请基于已有信息继续推进，或稍后再次询问。' };
  }
  return { status: ToolResultStatus.Ok, output: '用户已在对话框答复，见你的下一条用户消息。' };
}
